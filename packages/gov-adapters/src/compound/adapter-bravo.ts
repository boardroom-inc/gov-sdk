...
import governanceAbi from './abi-bravo.json';
...
interface CompoundGovernorBravoAdapterConfig {
 ...
 */
export class CompoundGovernorBravoAdapter implements ProposalsAdapter, VoteAdapter, DelegationAdapter, VotePowerAdapter, CreateOnChainProposalAdapter, GeneralAdapter {
  ...
  constructor(config: CompoundGovernorBravoAdapterConfig) {
  ...
  async getProposals(pagination: PaginationOptions = {}): Promise<ProposalPage> {
    const governor = new CompoundGovernorBravoContract(this.governanceAddress, this.transports);
  ...
  async getVotes(pagination: PaginationOptions = {}): Promise<VotePage> {
    ...
    const votes = mapVotes(results.items, this.isTokenERC721);
  ...
  async delegateVotingPower(delegatee: string, identifier?: string): Promise<string> {
    const signer = this.transports('signer').signer;
    let contract = new Contract(this.tokenAddress, compTokenAbi, signer);
    if (this.alternateVoteDelegatorAddress) {
      contract = new Contract(this.alternateVoteDelegatorAddress, compTokenAbi, signer);
    }
  ...
  async getDelegationEvents(pagination: PaginationOptions = {}): Promise<DelegationEventPage> {
    let governor = new CompoundGovernorBravoContract(this.tokenAddress, this.transports);
    if (this.alternateVoteDelegatorAddress) {
      governor = new CompoundGovernorBravoContract(this.alternateVoteDelegatorAddress, this.transports);
    }
    ...
    const events = results.items.map((i: LogResult<DelegateChangeEvents>) =>
      mapDelegationEvents(i, this.isTokenERC721)
    );
  ...
  async getDelegation(address: string): Promise<string> {
    const rpc = this.transports('rpc').network(await this.getChainId());
    let contract = new Contract(this.tokenAddress, compTokenAbi, rpc);
    if (this.alternateVoteDelegatorAddress) {
      contract = new Contract(this.alternateVoteDelegatorAddress, compTokenAbi, rpc);
    }
  ...
  formatTokenAmount = (balance: any) => {
    let formattedBalance = parseFloat(formatUnits(balance, 18));
  ...
  async getVotePower(addresses: string[], blockHeight?: number): Promise<VotePowerInfo[]> {
    ...
    let contract = new MultiContract(this.tokenAddress, compTokenAbi);
    if (this.alternateVoteDelegatorAddress) {
      contract = new MultiContract(this.alternateVoteDelegatorAddress, compTokenAbi);
    }
